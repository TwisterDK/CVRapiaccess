...
const UpdateCVRinfo = async (): Promise<string> => {
  ...
  for await (const company of companies) {
    // if (company.Companies_IDNo === '31596106') {
    ...
    // const mappedCVRitem = await mapcvrdata(Number(company.Companies_IDNo), cvrData);

    // console.log('Enhedsnummer:', mappedCVRitem.enhedsNummer);
    // }
  }
  const result = ` done `;
  return result;
